...
public class SaveUserCalendarToDevNexusTask extends AsyncTask<Void, Void, Void> {

    private final Context context;
    private final ContentResolver resolver;

    public SaveUserCalendarToDevNexusTask(Context context, ContentResolver resolver) {
    ...
    protected Void doInBackground(Void... params) {
        try {
            ...
            Set<Long> items = new HashSet<>(300);
            {
                Cursor userItemCursor = resolver.query(UserCalendarContract.URI, null, null, null, null);
                while (userItemCursor.moveToNext()) {
                    UserCalendar userItem = GsonUtils.GSON.fromJson(userItemCursor.getString(0), UserCalendar.class);

                    for (ScheduleItem si : userItem.items) {
                        if (si.presentation != null) {
                            items.add(Long.valueOf(si.presentation.id));
                        }
                    }
            ...
            if (items == null || StringUtils.isEmpty(googleToken)) {//Not logged in, do nothing.
                return null;
            }
            ...
            Request request = new Request.Builder()
                    ...
                    .post(RequestBody.create(MediaType.parse("text/json"), GsonUtils.GSON.toJson(items)))
